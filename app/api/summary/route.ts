import { NextResponse } from 'next/server';
import { getZoneSummary } from '@/lib/issues';
import { Zone } from '@/lib/schemas';

export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const zone = searchParams.get('zone') as Zone | null;

    if (!zone) {
      return NextResponse.json({ error: 'Missing zone parameter' }, { status: 400 });
    }

    const summary = await getZoneSummary(zone);

    return NextResponse.json({ success: true, zone, summary });
  } catch (error) {
    console.error('Zone summary error:', error);
    return NextResponse.json({ error: 'Failed to get zone summary' }, { status: 500 });
  }
}
